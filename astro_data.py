...
def load_iss_data(override):
    try:    
        # Check GitHub repo commit age instead of local file
        if override:
            owner = "NoahJens"
            ...
            age_days = (datetime.now(timezone.utc) - commit_dt).total_seconds() / 86400  # seconds → days
            print(age_days)
            if age_days >= 0.0001:  # 0.1 days ≈ 2.4 hours
                print("pre download")
